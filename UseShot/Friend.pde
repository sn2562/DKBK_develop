...
public class Friend {
	...
	public void update(PGraphics cv) {
		...
		cv.rotateY(data.get(tool.nowDataNumber).rotY);
		cv.rotateY(data.get(tool.nowDataNumber).rotZ);
		cv.scale(0.575f);
...
public class MyClient {
	...
	public MyClient(PApplet ap) {
		...
		if (!client.active())
			println("接続に失敗。サーバーの起動と設定を確認してください");
		else {
			println("接続に成功しました");
		}
	}
	...
	public void update(PGraphics cv) {
		...
		for (Integer id : friends.keySet ()) friends.get(id).update(cv);
	}
